...
export class AdminAPI {
  private static validateTopic(data: Record<string, unknown>): Topic | null {
    ...
    if (!Array.isArray(tags)) {
      console.error(`Tags must be an array: ${String(tags)}`);
      return null;
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        headers.forEach((header, index) => {
          ...
          if (
            header === 'tags' ||
            header === 'learning_outcomes' ||
            header === 'prerequisites' ||
            header === 'related_portfolio_slug'
          ) {
        ...
        results.errors.push(`Row ${i + 1}: Parse error - ${String(error)}`);
      }
    }
